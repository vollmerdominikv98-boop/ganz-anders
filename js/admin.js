...
export function renderAdminToolTable(db) {
    ...
    if (toolEntries.length === 0) {
        tbody.innerHTML = `<tr><td colspan="4" class="p-4 text-center text-slate-400 italic">Keine Werkzeuge gefunden</td></tr>`;
        return;
    }
    ...
    toolEntries.forEach(([id, t]) => {
        ...
        const hasMfrData = typeof t.mfr_ap_slot === 'number' && t.mfr_ap_slot > 0 && typeof t.mfr_ap_light === 'number' && t.mfr_ap_light > 0;
        const dataSourceBadge = hasMfrData
            ? `<span class="bg-indigo-100 text-indigo-800 px-1.5 py-0.5 rounded font-bold text-[10px]" title="${t.mfr_source ? escapeHTML(t.mfr_source) : ''}">📋 Herstellerwert</span>`
            : `<span class="bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded font-bold text-[10px]" title="Keine Herstellerdaten hinterlegt - App nutzt Faustregel">📐 Faustregel</span>`;
        ...
        tr.innerHTML = `
            ...
            <td class="p-2.5">${dataSourceBadge}</td>
...
export function editToolFromAdmin(id, db) {
    ...
    document.getElementById('adm-tool-mfr-ap-slot').value = t.mfr_ap_slot || '';
    document.getElementById('adm-tool-mfr-ap-light').value = t.mfr_ap_light || '';
    document.getElementById('adm-tool-mfr-source').value = t.mfr_source || '';
    toggleToolGeoFields();
...
export function resetToolForm() {
    ...
    document.getElementById('adm-tool-mfr-ap-slot').value = '';
    document.getElementById('adm-tool-mfr-ap-light').value = '';
    document.getElementById('adm-tool-mfr-source').value = '';
}
...
export async function saveToolFromForm(db, onSave) {
    ...
    const mfrApSlot = parseFloat(document.getElementById('adm-tool-mfr-ap-slot')?.value);
    const mfrApLight = parseFloat(document.getElementById('adm-tool-mfr-ap-light')?.value);
    db.tools[id] = {
        ...
        // Herstellerdaten für die ap-Grenze (Ebene 1) - optional, nur übernehmen wenn beide Werte gültig sind
        mfr_ap_slot: !isNaN(mfrApSlot) && mfrApSlot > 0 ? mfrApSlot : undefined,
        mfr_ap_light: !isNaN(mfrApLight) && mfrApLight > 0 ? mfrApLight : undefined,
        mfr_source: document.getElementById('adm-tool-mfr-source')?.value.trim() || undefined
    };
