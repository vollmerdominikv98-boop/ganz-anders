...
export function runMatchmaker(db, machId, matId, rigId) {
    ...
    validTools.forEach(toolId => {
        ...
        const res = calculatePhysics({
            db, machId, matId, profId, toolId, rigId, holderType: 'spannzange', 
            physicsActive: true, isRegrind: false, ap: ap_sim, ae: ae_sim
        });
        ...
        if (res) {
            ...
            rankedResults.push({
                toolId: toolId, brand: tool.brand, toolName: tool.name, diameter: D,
                q: res.q, score: effectiveScore, rpm: res.rpm, vf: res.vf,
                ap: ap_sim, ae: ae_sim, passes: passes, power: res.power,
                hasChatter: res.hasChatter, isFluteExceeded: res.isFluteExceeded,
                lc_max: res.lc_max, ap_krit: res.ap_krit
            });
        }
    });
